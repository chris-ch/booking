...
export class Interval {
  ...
  isSameAs(other: Interval): boolean {
    return this.low === other.low && this.high === other.high;
  }
}

function height<T extends Interval>(node?: Node) {
  if (node === undefined) {
    return -1;
  } else {
    return node.height;
...
export class Node {
  ...
  */
  ...
  private _getOverlappingRecords(currentNode: Node, low: number, high: number) {
    if (high > currentNode.key && low < currentNode.getNodeHigh()) {
...
export class IntervalTree {
  ...
  public insert(low: number, high: number) {
    if (low > high) {
      throw new Error('`low` value must be lower or equal to `high` value');
    }
    const record = new Interval(low, high);
    if (this.root === undefined) {
      ...
      if (node !== undefined) {
        ...
        for (let i = 0; i < node.records.length; i++) {
          if (node.records[i].isSameAs(record)) {
  ...
  public findIntervalFrom(low: number): Interval {
    const node = this.root.searchExisting(low);
    if (node) {
        return new Interval(low, node.getNodeHigh());
    } else {
        return null;
    }
  }

  public remove(low: number, high: number) {
    if (this.root === undefined) {
      ...
      if (node === undefined) {
        ...
        if (node.records[0].isSameAs(record)) {
...
export class InOrder implements IterableIterator<Interval> {
  private stack: Node[] = [];
...
export class PreOrder implements IterableIterator<Interval> {
  private stack: Node[] = [];
